    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 px-6 py-4 border-b border-blue-200 flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900">Detail Production Output</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-3 text-gray-600">Loading detail...</span>
            </div>
          ) : data && data.length > 0 ? (
                ...
                <tbody>
                  {data.map((item, idx) => (
                    <tr key={idx} className="border-b border-gray-200 hover:bg-blue-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{item.operation_code || item.op_code || '-'}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-right font-semibold text-green-600">{item.output || 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
